...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            #loop through free voxels along ray
            #occupied voxel
            hitpoint_x, hitpoint_y = sensor_x + r * np.cos(angle), sensor_y + r * np.sin(angle)
            hpvoxel_x, hpvoxel_y = self.PointToVoxel(hitpoint_x, hitpoint_y)
            self._map[hpvoxel_x][hpvoxel_y] = min(self._map[hpvoxel_x][hpvoxel_y] + self._occupied_update, self._occupied_threshold)

            increment_value = msg.range_max / 10
            for dist in np.arange(0, r, increment_value):
                point_x, point_y = sensor_x + dist * np.cos(angle), sensor_y + dist * np.sin(angle)
                voxel_x, voxel_y = self.PointToVoxel(point_x, point_y)
                if voxel_x != hpvoxel_x and voxel_y != hpvoxel_y:
